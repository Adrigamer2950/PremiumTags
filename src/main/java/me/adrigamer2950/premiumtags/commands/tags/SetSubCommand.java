...
public class SetSubCommand extends SubCommand {
    ...
    public boolean execute(CommandSender sender, String label, String[] args) {
        ...
        TagsManager.setTagToPlayer(p, tag);
        sender.sendMessage(Colors.translateColors("&aTag added successfully"));
    ...
    public List<String> tabComplete(@NotNull CommandSender sender, @NotNull String label, @NotNull String[] args) {
        ...
        if(args.length < 4)
            return TagsManager.getTagList().stream().map(Tag::getId).filter(id -> id.toLowerCase().startsWith(args[2])).collect(Collectors.toList());
